...
protected:
  void SetUp() override {
    ...
    auto tx_ = tx_graph_->BeginTransaction(memgraph::IsolationLevel::READ_UNCOMMITTED).ValueOrDie();

    // Initialize test data
    std::vector<memgraph::Node> initial_nodes = {memgraph::Node(1, {"Kv"}, "{\"key\": 1, \"value\": 10}"),
                                                 memgraph::Node(2, {"Kv"}, "{\"key\": 2, \"value\": 20}")};
    ASSERT_TRUE(tx_.AddNodes(initial_nodes).ok());
    auto status = tx_.Commit();
    ASSERT_TRUE(status.ok()) << "Failed to commit initial transaction: " << status.ToString();
  }
...
TEST_F(HermitageTest, G0) {
  ...
  // NOTE: If the next line succeeded (stading of changes done) -> there should be an error on commit.
